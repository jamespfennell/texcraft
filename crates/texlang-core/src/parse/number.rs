...
fn parse_optional_signs<T: Stream>(stream: &mut T) -> anyhow::Result<Option<Token>> {
    let mut result = None;
...
fn parse_character<S: Stream, T: PrimInt>(stream: &mut S) -> anyhow::Result<T> {
    match stream.next()? {
...
fn parse_octal<S: Stream, T: PrimInt>(stream: &mut S) -> anyhow::Result<T> {
...
fn parse_decimal<S: Stream, T: PrimInt>(stream: &mut S, n_start: i8) -> anyhow::Result<T> {
...
fn parse_hexadecimal<S: Stream, T: PrimInt>(stream: &mut S) -> anyhow::Result<T> {
